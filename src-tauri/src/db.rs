...
impl Database {
    ...
    pub fn get_all_sessions(&self) -> Result<Vec<Session>> {
        let conn = self.conn.lock().expect("Database lock poisoned");
        let mut stmt = conn.prepare(
            "SELECT id, project_id, started_at, ended_at, minutes FROM sessions ORDER BY started_at DESC"
        )?;
    ...
    pub fn get_all_daily_records(&self) -> Result<Vec<DailyRecord>> {
        let conn = self.conn.lock().expect("Database lock poisoned");
        let mut stmt = conn.prepare(
            "SELECT id, date, content, created_at, updated_at FROM daily_records ORDER BY date DESC"
        )?;
